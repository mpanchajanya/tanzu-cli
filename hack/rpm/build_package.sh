...
EOF
   ...
   reposync --repoid=tanzu-cli --download-metadata -p ${OUTPUT_DIR} --norepopath --source -y
   # Remove the old signature, which won't be valid anymore
   rm ${OUTPUT_DIR}/repodata/repomd.xml.asc
   
   # Now list the existing RPMs so we can pretend to have them locally
   for p in $(reposync --repoid=tanzu-cli -u -y | grep ${RPM_METADATA_BASE_URI}); do

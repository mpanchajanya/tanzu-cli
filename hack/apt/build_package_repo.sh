...
Origin: . tanzu-cli-jessie
Label: . tanzu-cli-jessie
Archive: tanzu-cli-jessie
Architecture: ${arch}
Component: main
EOF
...
Origin: . tanzu-cli-jessie
Label: . tanzu-cli-jessie
Suite: tanzu-cli-jessie
Codename: tanzu-cli-jessie
Date: $(date)
Architectures: amd64 arm64
Components: main
Description: Generated by a vmware custom script
MD5Sum:
$(for f in $(find main -type f); do echo "  $(md5sum $f)"; done)
SHA1:
$(for f in $(find main -type f); do echo "  $(sha1sum $f)"; done)
SHA256:
$(for f in $(find main -type f); do echo "  $(sha256sum $f)"; done)
SHA512:
$(for f in $(find main -type f); do echo "  $(sha512sum $f)"; done)
EOF
